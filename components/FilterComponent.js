...
export default {
	template: `
		<div>
			<h3>Filter</h3>
			<input type="text" v-model="filter_string" placeholder="Enter condition here...">
	...
	methods: {
		...
		apply() {
			try {
				const tokens = tokenize(this.filter_string);
				// console.log(tokens.map(a => ("value" in a) ? `${a.type}(${a.value})` : a.type).join(" "));
				const ast = parse(tokens);
				this.mask = evaluate(ast, this.dataset);
			} catch (error) {
...
function parse(tokens) {
	...
	if (pos < tokens.length) {
		throw new Error("Unexpected tokens remaining at position " + pos);
	}
